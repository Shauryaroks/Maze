        ...
        while current_position != self.end:
            ...
            for direction in self.moves:
                ...
                if is_valid and (next_x,next_y) not in visited:
                    ...
                    probabilities.append(pheremone_level*heuristic_level)
            #backtrack 
            if valid_moves == []:
                path.pop()
        ...
        while current_position != self.end:
            ...
            for direction in self.directions:
                ...
                if is_valid and ((next_x,next_y) not in visited) and ((next_x,next_y) ):
                    ...
                    probabilities.append(heuristic_level)
            #backtrack
            if valid_moves == []:
                path.pop()
                current_position = path[-1]             
                ...
                if is_valid and ((next_x,next_y) not in visited) and ((next_x,next_y)):
                    if ((next_x,next_y) not in p1) and ((next_x,next_y) not in p2):
                        ...
                        if ((next_x,next_y) in p1) and ((next_x,next_y) in p2):
                            ...
                            probabilities_inherited.append((min(len(p1)-p1.index((next_x,next_y)),len(p2)-p2.index((next_x,next_y))))*heuristic_level)
                        else:
                            if (next_x,next_y) in p1:
                                valid_inherited_moves.append((next_x,next_y))
                                heuristic_level = heuristic_level = (1/self.heuristic(next_x,next_y)+1e-5)
                                probabilities_inherited.append((len(p1)-p1.index((next_x,next_y)))*heuristic_level)
                            else:
                                valid_inherited_moves.append((next_x,next_y))
                                heuristic_level = heuristic_level = (1/self.heuristic(next_x,next_y)+1e-5)
                                probabilities_inherited.append((len(p2)-p2.index((next_x,next_y)))*heuristic_level)
...
s1 = time.time()
solution = AntColonySolver(m.maze_map,start,end, rows,columns)
...
s2 = time.time()
GA_solution = GeneticAlgorithmSolver(m.maze_map,start,end,rows,columns)
